...
- `game.js` depends on specific DOM element IDs (`board`, `next-canvas`, `score`, `lines`, `level`, `combo`, `overlay`, `overlay-title`, `overlay-score`, `overlay-records`, `restart-btn`, `name-form`, `name-input`, `start-screen`, `start-records`, `play-btn`, `reset-records-btn`, `theme-toggle`, `skin-select`, `pause-menu`, `resume-btn`, `menu-restart-btn`, `controls-btn`, `menu-controls`, `start-level`). Renaming in HTML requires matching updates in the `getElementById` calls at the top of `game.js`.
- `localStorage` keys: `tetris-records` (top 5), `tetris-last-name`, `tetris-theme`, `tetris-skin`, `tetris-start-level`. `loadRecords()` sanitizes/sorts/truncates on every read, so corrupt or hand-edited data degrades to `[]` instead of throwing.
